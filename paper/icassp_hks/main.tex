...
\begin{figure*}[t]
\centering
\includegraphics[width=0.72\textwidth]{A1_actor_augmentation_overview.pdf}
\caption{Actor-side structural augmentation. Each actor retains its 18-dimensional raw observation, constructs the observation-derived agent--landmark relation graph, computes three focal HKS values, and concatenates them with the raw actor input. The centralized critic continues to use raw joint observations and actions.}
...
Let $D_i=\operatorname{diag}(W_i\mathbf{1})$ and
\begin{equation}
...
Raw MADDPG and HKS-augmented MADDPG are trained with seeds 41--45 for $2\times10^6$ environment transitions per seed. Both use four rollout environments, replay capacity $10^6$, batch size 1024, discount 0.95, actor/critic learning rate 0.01, target-update coefficient 0.01, and two 64-unit ReLU hidden layers. For each training seed, checkpoints are evaluated every 20,000 steps on a common 100-episode selection bank; the checkpoint with the highest mean return is then evaluated without exploration on a disjoint 500-episode bank.
